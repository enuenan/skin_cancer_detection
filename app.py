...
labels = {1: "No Cancer", 0: "Cancer Detected"}

# Define a function to preprocess the image
def preprocess_image(image_path):
    # Open the image file
    image = Image.open(image_path).convert('RGB')  # Convert the image to RGB
    # Resize the image to 150x150 pixels (match your model input size)
...
def index():
    if request.method == 'POST':
        ...
        return render_template('index.html', result=result, value=predicted_class, file_path=file.filename)
